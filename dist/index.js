...
let gerenciador = new GerenciadorDeProduto(); // cria um objeto de uma classe e armazena ele em uma variável com o tipo indicado
function adicionarProduto() {
    ...
    let codigo = Number(codigoInput.value.trim()); // .trim() remove espaços em branco no início e no final da string
...
function editarProduto(botao) {
    ...
    let codigo = Number(linha.cells[1].textContent);
    let quantidade = Number(linha.cells[2].textContent);
    let precoTexto = linha.cells[3].textContent || '';
    ...
    let botaoAdicionar = document.querySelector("button[onclick='adicionarProduto()']");
...
function salvarEdicao() {
    ...
    let botaoAdicionar = document.querySelector("button[onclick='salvarEdicao()']");
